...
En este taller veremos como graficar datos para visualización, sin embargo, los 
paquetes de R permiten hacer mucho más que solo 
...
argumento la ruta al archivo con terminación `.shp` y la asigna a `depart_uy`.
...
contenidas a la misma gráfica que los
polígonos.
...
También tenemos disponibles las opciones típicas de ggplot.
...
Leaflet recibe datos geoespaciales con la CRS 4326 (al igual que las coordenadas 
que copiamos de GoogleMaps).

Si queremos graficar los puntos culturales debemos comenzar reproyectando:
...
barrios_cultura_mv <- barrios_mv %>%
    st_join(cultura_mv, join = st_within) 
...
https://catalogodatos.gub.uy/dataset/ide-limites-departamentales
